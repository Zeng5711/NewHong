...
package com.hongbang.ic.bluetooth;
...
public class BluetoothLeService extends Service {
    ...
    private final BluetoothGattCallback mGattCallback = new BluetoothGattCallback() {
        ...
        public void onConnectionStateChange(BluetoothGatt gatt, int status, int newState) {
            ...
            if (newState == BluetoothProfile.STATE_CONNECTED) {
                ...
                Logger.info(TAG, "Attempting to start service discovery:" +
        ...
        public void onServicesDiscovered(BluetoothGatt gatt, int status) {
            if (status == BluetoothGatt.GATT_SUCCESS) {
                ...
                Logger.info(TAG, "onServicesDiscovered received: " + status);
        ...
        public void onCharacteristicChanged(BluetoothGatt gatt,
            ...
            broadcastUpdate(ACTION_DATA_AVAILABLE, characteristic);
            //数据
            Logger.info("xiawei", characteristic.getValue().toString());

            Logger.info("onCharacteristicChanged", "返回读出的值");
        }
    ...
    private void broadcastUpdate(final String action,
        ...
        if (UUID_HEART_RATE_MEASUREMENT.equals(characteristic.getUuid())) {
            ...
            if ((flag & 0x01) != 0) {
                ...
                Logger.info(TAG, "Heart rate format UINT16.");
            } else {
                format = BluetoothGattCharacteristic.FORMAT_UINT8;
                Logger.info(TAG, "Heart rate format UINT8.");
            }
            final int heartRate = characteristic.getIntValue(format, 1);
            Logger.info(TAG, String.format("Received heart rate: %d", heartRate));
    ...
    public boolean initialize() {
        ...
        if (mBluetoothManager == null) {
            ...
            if (mBluetoothManager == null) {
                Logger.info(TAG, "Unable to initialize BluetoothManager.");
                return false;
            }
        }
        ...
        if (mBluetoothAdapter == null) {
            Logger.info(TAG, "Unable to obtain a BluetoothAdapter.");
            return false;
        }
    ...
    public boolean connect(final String address) {
        if (mBluetoothAdapter == null || address == null) {
            Logger.info(TAG, "BluetoothAdapter not initialized or unspecified address.");
            return false;
        }
        ...
        if (mBluetoothDeviceAddress != null && address.equals(mBluetoothDeviceAddress)
                && mBluetoothGatt != null) {
            Logger.info(TAG, "Trying to use an existing mBluetoothGatt for connection.");
        ...
        if (device == null) {
            Logger.info(TAG, "Device not found.  Unable to connect.");
            return false;
        }
        // We want to directly connect to the device, so we are setting the autoConnect
        // parameter to false.
        mBluetoothGatt = device.connectGatt(this, false, mGattCallback);
        Logger.info(TAG, "Trying to create a new connection.");
    ...
    public void disconnect() {
        if (mBluetoothAdapter == null || mBluetoothGatt == null) {
            Logger.info(TAG, "BluetoothAdapter not initialized");
            return;
        }
    ...
    public void writeLlsAlertLevel(int iAlertLevel, byte[] bb) {
        ...
        Logger.info(TAG, "storedLevel() - storedLevel=" + storedLevel);


        alertLevel.setValue(bb);
        Logger.info("发送的指令", "bb" + bb[0]);

        alertLevel.setWriteType(BluetoothGattCharacteristic.WRITE_TYPE_NO_RESPONSE);
        status = mBluetoothGatt.writeCharacteristic(alertLevel);
        Logger.info(TAG, "writeLlsAlertLevel() - status=" + status);
    }
