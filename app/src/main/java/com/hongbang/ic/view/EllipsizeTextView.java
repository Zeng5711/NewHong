...
public class EllipsizeTextView extends TextView {
    ...
    public EllipsizeTextView(Context context, AttributeSet attrs) {
        super(context, attrs);
        maxLines = attrs.getAttributeIntValue(((XmlResourceParser) attrs).getAttributeNamespace(0), "maxLines", -1);
    }
    ...
    public int getMaxLines() {
        ...
        for (int i = 0; i < maxLines; i++) {
            Layout layout = createWorkingLayout(lines);
            if (layout.getHeight() > getHeight() - getPaddingBottom() - getPaddingTop()) {
                break;
            }
            limit = i + 1;
            lines += "\nl";
