...
public class ClipZoomImageView extends ImageView implements
    ...
    public boolean onScale(ScaleGestureDetector detector) {
        float scale = Math.max(getScale(), initScale);
    ...
    public Bitmap clip(int targetSize) {
        Bitmap bitmap = Bitmap.createBitmap(getWidth(), getHeight(),
                Bitmap.Config.ARGB_8888);
        Canvas canvas = new Canvas(bitmap);
        draw(canvas);
        Matrix matrix = new Matrix();
        if (targetSize > 0 && targetSize < bitmap.getWidth()) {
            float scale = 1.0f * targetSize / bitmap.getWidth();
    ...
    private void checkBorder() {
        ...
        if (rect.width() + 1 >= width - 2 * mHorizontalPadding) {
        ...
        if (rect.height() + 1 >= height - 2 * getHVerticalPadding()) {
